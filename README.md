...
Downloads the Minigraph-Cactus HPRC pangenome VCF, pops 10 Mb bubbles while keeping SV records, filters for REF/ALT length > 100 bp, splits multiallelic records, merges with Wenwei's repeat annotations, filters for TE annotations only, and reconstructs the VCF header.
...
## 1.2 Processing of the TE VCF

Normalizes/joins multiallelic records, splits autosomes from sex chromosomes and filters missing sites at 10% on autosomes, removes undefined-frequency sites on chrX/chrY, checks per-individual and per-site missingness, then removes duplicate entries and splits the result into biallelic (1 alternative) and multiallelic datasets.
...
Calculates population-specific allele frequencies (AFR vs. outofAFR vs. global), counts TE variants per individual (major, singleton, and shared categories), runs a linkage-pruned PCA in plink, and performs frequency-based (Fst), haplotype-based, and Ohana demography-based selection scans, followed by genotype/admixture matrix analyses.
...
Runs RepeatMasker on the biallelic TE VCF converted to FASTA, then estimates divergence and builds a repeat landscape, summary, and Kimura distance table for age estimation.
...
Runs ANNOVAR gene-based annotation on the TE VCF, then extracts candidate TE loci flagged by the selection scans and prepares them for upload to AnnotSV and VEP.
...
Generates random background genomic windows matched in number to the foreground TE set, then shuffles them excluding the foreground TE regions for recombination rate comparison.
...
The analysis is run on the Minigraph-Cactus HPRC pangenome VCF, hprc-v1.0-mc-grch38.vcf.gz, downloaded from the HPRC freeze1 release.
